...
private[logoot] object GenerateLineId {
  ...
  private def gen(p: LineId, q: LineId, n: Int, boundary: Option[Int]): Logoot[List[LineId]] = {
    ...
    for {
      sid  <- LogootOp.site
      _    <- LogootOp.tick
      time <- LogootOp.timeNow
      nums <- numList.traverseU { num => LogootOp.rand(stepNum).map(_ + num + Number.One) }
  ...
  def apply(p: LineId, q: LineId, n: Int, boundary: Option[Int]): Logoot[List[LineId]] = {
    ...
    if ((p >= q) || (n < 1) || qIsZero) Logoot.point(List.empty[LineId])
    else gen(p, q, n, boundary.filter(_ > 0))
  }
}
